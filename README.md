...
- 管理頁（給管理者）：`admin.html`，用 Google 帳號登入後可以匯入新學期課表、管理教室和學期、查看或匯出預約紀錄。
...
| 管理者登入 | Firebase Authentication（Google 登入） |

`js/firebase.js` 裡的連線設定本來就是公開的，資料安全由 `firestore.rules` 控管：

- 任何人都可以看課表和預約、新增預約，也能用相同的姓名取消自己的預約。
- 同一格（日期＋節次＋教室）只能有一筆有效預約。
- 只有 Firestore `admins` 集合裡的 Email 可以修改教室、學期、固定課表，並取消任何預約。

## 新增管理者

Firebase 主控台 → Firestore Database → `admins` 集合 → 新增文件，文件 ID 填管理者的 Google Email，欄位隨意，例如 `note: 管理者`。
